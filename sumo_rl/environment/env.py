...
class SumoEnvironment:
    ...
    def _start_simulation(self):
        sumo_cmd = [self._sumo_binary,
                    ...
                    # add by longshui, generate tripinfo files
                    '--tripinfo-output', '/home/longshui/Desktop/logs/trip_ql_heavy.xml']
    ...
    def step_long(self, action, seconds):
        # No action, follow fixed TL defined in self.phases
        # if action is None or action == {}:
        #     for _ in range(self.delta_time):
        #         self._sumo_step()
        self._sumo_step()
        if 0 <= seconds % 152 < 40:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GGGGrGrrrrGGGGrGrrrr")
        elif 40 <= seconds % 152 < 43:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GyyyrGrrrrGyyyrGrrrr")
        elif 43 <= seconds % 152 < 73:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GrrrGGrrrrGrrrGGrrrr")
        elif 73 <= seconds % 152 < 76:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GrrryGrrrrGrrryGrrrr")
        elif 76 <= seconds % 152 < 116:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GrrrrGGGGrGrrrrGGGGr")
        elif 116 <= seconds % 152 < 119:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GrrrrGyyyrGrrrrGyyyr")
        elif 119 <= seconds % 152 < 149:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GrrrrGrrrGGrrrrGrrrG")
        else:
            traci.trafficlight.setRedYellowGreenState("gneJ0", "GrrrrGrrryGrrrrGrrry")
    ...
    def _apply_actions(self, actions):
        ...
        if self.single_agent:
            if self.traffic_signals[self.ts_ids[0]].time_to_act:
                self.traffic_signals[self.ts_ids[0]].set_next_phase(actions) #设置下一个绿色相位，如果下一个相位与当前相位不同，则设置黄色相位
        else:
    ...
    # add by longshui: #for 6lanes and fixed_6lanes
    def _compute_observations(self):
        self.observations.update({ts: self.traffic_signals[ts].compute_observation_long1() for ts in self.ts_ids if self.traffic_signals[ts].time_to_act})
        return {ts: self.observations[ts].copy() for ts in self.observations.keys() if self.traffic_signals[ts].time_to_act}
    ...
    def encode_long(self, state, ts_id):
        phase = int(np.where(state[:self.traffic_signals[ts_id].num_green_phases] == 1)[0])
        min_green = state[self.traffic_signals[ts_id].num_green_phases]
        density_queue = [self._discretize_density(d) for d in state[self.traffic_signals[ts_id].num_green_phases + 1:]]
        # relative_waiting_time = self.
        # tuples are hashable and can be used as key in python dictionary
        return tuple([phase, min_green] + density_queue)
