...
class TrafficSignal:
    ...
    def set_next_phase(self, new_phase):
        ...
        设置下一个绿色相位，如果下一个相位与当前相位不同，则设置黄色相位
    ...
    def compute_observation(self):
        ...
        # print(traci.edge.getWaitingTime(edgeID="gneE1"))
        # print(traci.edge.getWaitingTime(edgeID="-gneE0"))
        # print(traci.edge.getWaitingTime(edgeID="gneE3"))
        # print(traci.edge.getWaitingTime(edgeID="gneE2"))
        # print()
        # single intersection-6lanes: phase_id:list4, min_green: list1, density:list12  queue:list12
        observation = np.array(phase_id + min_green + density + queue, dtype=np.float32)
        # print(phase_id)
        return observation

    # phase_id + min_green + waitingtime
    def compute_observation_long(self):
        phase_id = [1 if self.green_phase == i else 0 for i in range(self.num_green_phases)]  # one-hot encoding
        min_green = [0 if self.time_since_last_phase_change < self.min_green + self.yellow_time else 1]

        # east west south north
        a = int(traci.edge.getWaitingTime(edgeID="gneE1"))
        b = int(traci.edge.getWaitingTime(edgeID="-gneE0"))
        c = int(traci.edge.getWaitingTime(edgeID="gneE3"))
        d = int(traci.edge.getWaitingTime(edgeID="gneE2"))
        list1 = [a, b, c, d]
        observation = np.array(phase_id + min_green + list1, dtype=np.int64)
        # print(phase_id)
        return observation


    # phase_id + min_green + relative waitingtime
    def compute_observation_long1(self):
        phase_id = [1 if self.green_phase == i else 0 for i in range(self.num_green_phases)]  # one-hot encoding
        min_green = [0 if self.time_since_last_phase_change < self.min_green + self.yellow_time else 1]
        ...
        list1 = [a, b, c, d]
        t = [[0, a], [1, b], [2, c], [3, d]]
        t1 = sorted(t, key=lambda x: (x[1], -x[0]))
        for i in range(4):
            list1[t1[i][0]] = i + 1
        # print(list1)
        observation = np.array(phase_id + min_green + list1, dtype=np.int64)
    ...
    def get_lanes_queue(self):
        vehicle_size_min_gap = 7.5  # 5(vehSize) + 2.5(minGap)
        # for lane in self.lanes:
        #     print(lane) #str gneE2_0
        #     print(type(lane))
        #     print(self.lanes_lenght[lane])
